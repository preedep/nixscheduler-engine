...
async fn main()  -> std::io::Result<()> {
    ...
    let shard: Arc<dyn ShardManager> = match &app_conf.shard_mode {
        config::ShardMode::Distributed { shard_id, total_shards } => {
            Arc::new(DistributedShardManager::new(*shard_id, *total_shards, app_conf.clone().into()))
        }
        config::ShardMode::Local => {
            Arc::new(LocalShardManager::new(app_conf.clone().into()))
        }
    };
